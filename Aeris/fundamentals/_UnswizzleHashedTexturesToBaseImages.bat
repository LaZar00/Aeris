...
set "inputfolder=D:\Coding\_Aeris\_ANIMS"
set "outputfolder=D:\Coding\_Aeris\_ANIMSUNSWIZZLED"
...
if NOT EXIST %inputfolder% (
	echo( 
	echo Dumped Hashed Anims Folder: %inputfolder% does not exists.
	goto exit
)

if NOT EXIST %outputfolder% (
	echo(
	echo Unswizzled Hashed Anims Folder: %outputfolder% does not exists.
	goto exit
)
...
for /D %%d in (%inputfolder%\*) do (
	...
	aeris -u %fieldsfolder%\%%~nd %inputfolder%\%%~nd %outputfolder%\%%~nd -v
	echo =================== END =========================
)
